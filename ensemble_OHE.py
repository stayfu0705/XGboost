...
def bagging():
    ...
    mlist = pd.unique(data['MD_NO'])

    # 第五台沒資料
    for m in mlist:
        pno = {405108: 50, 405568: 23, 405984: 11, 406750: 83}
        # pno = {405108: 54, 405568: 23, 405984: 12, 406750: 89}
        temp = data[data['MD_NO'] == m].copy()
        X_train, X_test, y_train, y_test = train_test_split(
            temp.drop('diff', axis=1), temp['diff'], test_size=0.3, random_state=42
        )
...
def xgboost():
    ...
    X_train, X_test, y_train, y_test = train_test_split(
        data.drop('diff', axis=1), data['diff'], test_size=0.3, random_state=42
    )
